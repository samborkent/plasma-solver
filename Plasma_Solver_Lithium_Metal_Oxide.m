...
% Save location
Dir         = ['C:\Users\Sam\Google Drive\School\Master\Capita selecta' ...
...
% Velocity settings
V_MIN   = X_Delta / T_Delta; % Minimal initial velocity
V_Max   = 40000;             % Maximal initial velocity
V_Delta = 5;                 % Velocity step size 0 : (V_MIN/V_Delta) : V_Max;
v = 0 : (V_MIN / V_Delta) : V_Max;
V_NUMEL = numel(v);
...
% Atomic masses
M_A = 87.62  * AMU;   % Mass A atom [kg]
M_B = 47.867 * AMU;   % Mass B atom [kg]
M_O = 15.999 * AMU;   % Mass O atom [kg]

% Array holding all possible plume compounds
M_Array = [M_A M_B M_O 2*M_O M_A+M_O M_B+M_O M_B+2*M_O 2*M_B+3*M_O]; % Sr Ti O O2 SrO TiO TiO2 Ti2O3
M_NUMEL = numel(M_Array);
...
% Unit cell
UC_N_Atoms      = [1 1 3];          % Amount of each atom in unit cell [A B O]
UC_N_NUMEL      = numel(UC_N_Atoms);
UC_Vol          = (3.945E-10)^3;    % Volume of SrTiO3 unit cell [m3]
UC_E_Formation  = 3.5615 * EV;      % Formation energy [J]

% Optical properties
Adsorption_Coeff = 0.77; % Adsorbtion coefficient SrTiO3 (Temp and laser energy dependend) (check value)

% Target
%?%Density_Target = 1;  % Target density

% Energies
E_TiO = 0.117 * EV;                 % Activation energy for forming TiO [J]
E_O_low = 0.5 * EV;                 % Lower bound oxidation energy [J]
E_O_high_Sr = 4.418 * EV; %* Oxidize; % Upper bound oxidation energy Sr [J]
E_O_high_Ti = 2.276 * EV; %* Oxidize; % Upper bound oxidation energy Ti [J]

Q = 0;  % No significant heat loss into the target (ceramic)
dE_Excitation = zeros(1, M_NUMEL);   % No significant excitation of species
...
V_Dis_Width = 1654;         %?% Width velocity distribution
...
DENSITY_BG  = P_BG_PASCAL / (BOLTZMANN * Temp_Bg);    % Density N/V of background gas (ideal gas law)

% Laser
SPOT_VOLUME = Spot_Width * Spot_Height * Spot_Depth;    % Laser spot volume [m3]

% Unit cell
UC_N_TOTAL      = sum(UC_N_Atoms);              % Total number of atoms in unit cell
UC_E_BINDING    = UC_N_TOTAL * UC_E_Formation;  % Binding energy crystal (5 times formation energy per atom) [eV]

%--------------------------------------------------------------------------
%% Initial ditribution
N_UC = SPOT_VOLUME / UC_Vol;
...
n_atom_rad = (n_atom_temp .* N_UC) ./ sum(n_atom_temp); %?%

%--------------------------------------------------------------------------
%% Write settings into config file

% Make directory if none exists
if ~isfolder([Dir Folder])
    mkdir([Dir Folder])
end

% Generate file name
file_name = [Dir Folder '\' Output_File '_' datestr(now, 'yyyy-mm-dd_HHMM') '.txt'];
...
fprintf(fid, ['\tCONFIGURATION FILE\t\t' datestr(now, 'yyyy-mm-dd\tHH:MM') '\n\n' ...
    ...
    'Velocity limits [m/s]:' '\t\t\t' 'min ' num2str(V_MIN) ', max ' num2str(V_Max) ', dv ' num2str(V_Delta) '\n' ...
...
%--------------------------------------------------------------------------
%% Pre-allocations
v_dis_initial = zeros(1, V_NUMEL);  % Initial velocity distribution
E_k = zeros(1, M_NUMEL);
...
for theta = 1 : 1 %(numel(rad) - 1)
    ...
    %% Initial particle verlocity distribution
    % Loop over species in the plume
    for atoms = 1 : UC_N_NUMEL
        %?% Waarom delen door adsorptie coeff?
%         v_gem_test = sqrt((dis_E_x / (A_coeff * N_atoom_totaal / sum(T_sum)) - Eb_UC) * (2/(sum(T_sum)*m_P(mm)))) %nieuwe regel 7-12 Bart %checken voor de 0.9
        v_avg_test = sqrt( ( 2*(E_Laser * Adsorption_Coeff) / (N_UC / sum(UC_N_Atoms)) ...
            - UC_E_Formation) / (sum(UC_N_Atoms) * M_Array(atoms)) ); 

        % Average velocity per species %?% 0.9
        v_avg_tom = sqrt( 2*((E_Laser * Adsorption_Coeff) / (N_UC / sum(UC_N_Atoms)) ...
            - UC_E_Formation) / M_Array(atoms) );
        
        % Initial kinetic energy of species
        E_k(atoms) = (((E_Laser / Adsorption_Coeff) - Q) / N_UC) ...
            - UC_E_Formation - dE_Excitation(atoms);
        
        % Average velocity
        v_c = sqrt(2 * E_k(atoms) / M_Array(atoms));
        
        for i = 1 : V_NUMEL
%             v_dis_initial(i) = exp(- (v(i) - v_avg_tom)^2 / (2 * V_Delta^2) ) / (V_Delta * sqrt(2*pi));
            v_dis_initial(i) = exp(-(v(i) - v_c)^2 / (2 * V_Dis_Width^2)) ... 
                / (V_Dis_Width * sqrt(2 * pi));
        end
        
        nor = N_UC;
        
        plot(v, v_dis_initial);
    end
end
