...
nPlasma = plasmaMatrix(iVelo, jRadius, nField);
...
kPlasma = plasmaMatrix(iVelo, jRadius, kField);
...
%   radial bins
if ( jRadius + nRadiusTraveled(iVelo) ) > nRadius
    nRadiusDelta = nRadius - jRadius;
else
...
% Loop from current bin to one bin before final bin
    ...
    for jVelo = 1 : bgVeloNumel
        ...
        % Prevent index out-of-range error
        if iNewPlasmaRadius(iBGVelo, iThisRadius) > nRadius
            iNewPlasmaRadius(iBGVelo, iThisRadius) = nRadius;
        end
        if iNewBGRadius(iBGVelo, iThisRadius) > nRadius
            iNewBGRadius(iBGVelo, iThisRadius) = nRadius;
        end
    ...
    % If the total number of collided particles is greater than the number of
    %   plasma particles
...
iNewPlasmaRadiusNonCol = jRadius + nRadiusDelta;
...
% Loop from current bin to one bin before final bin
    ...
    for jVelo = 1 : numel(bgVeloArray)
        ...
        plasmaNew(iNewVelo, jRadius + iNewPlasmaRadius(jVelo, iThisRadius), nField) = ...
            nCol(jVelo, iThisRadius);

        plasmaNew(iNewVelo, jRadius + iNewPlasmaRadius(jVelo, iThisRadius), kField) = 1 ...
            + plasmaNew(iNewVelo, jRadius + iNewPlasmaRadius(jVelo, iThisRadius), kField);
        ...
        bgMatrix(iNewBGVelo, thisRadius) = bgMatrix(iNewBGVelo, thisRadius) ...
            + nCol(iBGVelo, iThisRadius);
        
%         bgMatrix(iBGVelo, thisRadius) = bgMatrix(iBGVelo, thisRadius) ...
%             - nCol(iBGVelo, iThisRadius);
% 
%         bgMatrix(iNewBGVelo, iRadius + iNewBGRadius(jVelo, iThisRadius)) = ...
%             bgMatrix(iNewBGVelo, iRadius + iNewBGRadius(jVelo, iThisRadius)) ...
%             + nCol(iBGVelo, iThisRadius);
...
for jRadius = (nRadius - 1) : -1 : 1
    for jVelo = nVelo : -1 : iFirstVelo
        ...
        if ( jRadius + nRadiusTraveled(iVelo) ) > nRadius
            ...
            nRadiusDelta = nRadiusTraveled(jVelo);
        end

        % Move background particles to new position
        bgMatrix(jVelo, jRadius + nRadiusDelta) = bgMatrix(jVelo, jRadius);

        % Empty previous position
        bgMatrix(jVelo, jRadius) = 0;
